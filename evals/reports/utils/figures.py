...
def fig_cohen_d(data: dict | None = None) -> Path:
    ...
    labels = [
        f"★ {r['label']}" if r.get("is_default") else r["label"]
        for r in rows
    ]
    values = [r["d"] for r in rows]
    colors = [
        GREEN if v > 0.1 else TEAL if v > 0 else AMBER if v > -0.05 else RED
        for v in values
    ]
    ...
    _style()
    bars = ax.barh(
        labels, values, color=colors, height=0.55, zorder=3,
        edgecolor=edges, linewidth=linewidths,
    )
    ax.axvline(0, color=MID, linewidth=1.2, linestyle="--", alpha=0.7)
    ax.axvline(0.2, color=TEAL, linewidth=1.0, linestyle=":", alpha=0.6, label="small effect (d=0.2)")
    ...
    legend = [
        mpatches.Patch(facecolor=GREEN, edgecolor=NAVY, linewidth=2, label="★ Default gate (d > 0.1)"),
...
def fig_threshold_viz() -> Path:
    ...
    for i, (thr, lm, dm) in enumerate(zip(tholds, liked_m, dislike_m)):
        ax.plot([i - width, i + width], [thr, thr], color=NAVY, linewidth=2,
                linestyle="--", zorder=4)
    ...
    ax2.bar(x + width / 2, dislike_p, width, color=RED,   alpha=0.8, zorder=3, label="Disliked pass %")

    for i, (lp, dp) in enumerate(zip(liked_p, dislike_p)):
        gap = lp - dp
    ...
    ax2.set_ylim(0, 115)
    ax2.set_title("Pass Rate at Threshold: Liked vs Disliked\n(Δ = separation at current threshold)")
...
def _golden_heuristic_retrieval() -> dict:
    ...
    n_rated = (
        stats.get("sentiment", {}).get("n_liked", 0)
        + stats.get("sentiment", {}).get("n_disliked", 0)
    )
    return {
    ...
    for rec in records:
        ...
        for grader in _CORE_GRADER_KEYS:
            score = calibrated_score(
                grader, gr.get(grader), _grader_threshold(grader), ctx,
            )
...
def _golden_heuristic_rated_records_v2() -> list[dict]:
    ...
    if DEFAULT_BKH_ALL.exists():
        for row in compute_overlap_records(
            load_jsonl(resolve_golden_responses_path()), DEFAULT_BKH_ALL,
        ):
            overlap_by_id[row["task_id"]] = row

    tasks = {
        t.task_id: t
        for t in load_jsonl(resolve_golden_responses_path())
    }
    ...
    for rec in records:
        ...
        rec["bkh_va_overlap_v2_score"] = (
            float(same_source_adjusted(ov)) if ov else None
        )
    return records
...
def _heuristic_panel_specs(
    ...
    specs: list[tuple[str, str, float, float, float]] = []
    rw_scores = [
        r["response_words_score"] for r in records
        if r.get("response_words_score") is not None
    ]
...
def _plot_score_distribution_kde(
    ...
    def _plot_kde(
        scores: list[float], color: str, linestyle: str, fill_alpha: float, legend: str,
    ) -> None:
    ...
    if threshold > 0:
        ax.axvline(threshold, color=NAVY, linewidth=1.8, linestyle="--", alpha=0.85,
                   label=f"thr={threshold:g}")
        band = 0.08 if x_max <= 1.5 else max(2.0, (x_max - x_min) * 0.03)
        ax.axvspan(
            max(x_min, threshold - band), min(x_max, threshold + band),
...
def fig_box_plots() -> Path:
    ...
    for ax, (key, label, threshold, _, _) in zip(axes_flat, panels):
        l_scores = [r[key + "_score"] for r in records
                    if r["sentiment"] == "liked" and r.get(key + "_score") is not None]
        d_scores = [r[key + "_score"] for r in records
                    if r["sentiment"] == "disliked" and r.get(key + "_score") is not None]

        ax.boxplot(
            [l_scores], positions=[0], widths=0.4, patch_artist=True,
            boxprops=dict(facecolor=TEAL + "55", edgecolor=TEAL, linewidth=1.5),
            medianprops=dict(color=NAVY, linewidth=2),
            whiskerprops=dict(color=TEAL, linewidth=1.5),
            capprops=dict(color=TEAL, linewidth=1.5),
            flierprops=dict(marker="o", color=TEAL, alpha=0.25, markersize=3),
        )
        ax.boxplot(
            [d_scores], positions=[1], widths=0.4, patch_artist=True,
            boxprops=dict(facecolor="none", edgecolor=RED, linewidth=1.5, hatch="///"),
            medianprops=dict(color=RED, linewidth=2),
            whiskerprops=dict(color=RED, linewidth=1.5),
            capprops=dict(color=RED, linewidth=1.5),
            flierprops=dict(marker="o", color=RED, alpha=0.25, markersize=3),
        )
        ax.axhline(threshold, color=NAVY, linewidth=1.5, linestyle="--", alpha=0.75,
...
def fig_kde_thresholds() -> Path:
    ...
    for ax, (key, label, threshold, x_min, x_max) in zip(axes_flat, panels):
...
def fig_kde_thresholds_v2() -> Path:
    ...
    n_disliked = sum(1 for r in records if r["sentiment"] == "disliked")
    n_uplift = sum(
        1 for raw, cal in zip(raw_records, records)
...
def fig_kde_heuristic_thresholds() -> Path:
    ...
    for ax, (key, label, threshold, x_min, x_max) in zip(axes_flat, panels):
        ...
        if key == "bkh_va_overlap":
            ...
            n_labeled = sum(
                1 for r in records if r.get("weighted_resolution_score") is not None
            )
...
def fig_kde_heuristic_thresholds_v2() -> Path:
    ...
    d_hs_v1 = _heuristic_kde_cohens_d(
        _golden_heuristic_rated_records(), "has_source",
    )
    d_hs_v2 = _heuristic_kde_cohens_d(records, "has_source_v2")
    d_ov_v1 = _heuristic_kde_cohens_d(
        _golden_heuristic_rated_records(), "bkh_va_overlap",
    )
...
def fig_mrr_comparison(configs: dict | None = None) -> Path:
    ...
    if configs:
        items = sorted(configs.items(), key=lambda kv: kv[1].get("aggregate", {}).get("mrr", 0), reverse=True)
    ...
    colors = [
        GREEN if v > 0.57 else
        TEAL  if v > 0.52 else
        AMBER if v > 0.49 else
        MID   if v == 0.500 else
        RED
        for v in values
    ]
    ...
    ax.axvline(0.500, color=NAVY, linewidth=1.5, linestyle="--", alpha=0.6, label="Production ref (va_staging) 0.500")
...
def _pass_rate_chart(
    ...
    *,
    ...
    rates = [
        round(grader_summary.get(m[0], {}).get("pass_rate", 0) * 100)
        for m in metric_keys
    ]
    colors = [GREEN if r >= t else RED for r, t in zip(rates, thresholds)]
    ...
    ax.plot(x, thresholds, "o--", color=NAVY, linewidth=1.8, markersize=5, zorder=4, label="Threshold")
    ax.set_xticks(x)
    ...
    for bar, val in zip(bars, rates):
        ax.text(
            bar.get_x() + bar.get_width() / 2, val + 1.2,
            f"{val}%", ha="center", va="bottom", fontsize=8, color=NAVY, fontweight="bold",
        )
    if subtitle:
        fig.suptitle(
            f"{title}\n{subtitle}",
            fontsize=10, color=NAVY, fontweight="bold", y=0.98,
        )
    else:
...
def _golden_llm_pass_rates_figure(
    ...
    *,
    ...
    gs = compute_llm_pass_summary(
        quality, metric_keys, calibrated=calibrated, cal_index=cal_index,
    )
...
def fig_golden_pass_rates_v1(quality: dict | None = None) -> Path:
    ...
    return _golden_llm_pass_rates_figure(
        q, GOLDEN_LLM_METRICS_V1, version="v1", out_name="golden_pass_rates_v1", calibrated=False,
    )
...
def fig_golden_pass_rates_compare(quality: dict | None = None) -> Path:
    ...
    va_gs_v2 = compute_llm_pass_summary(
        q, GOLDEN_LLM_METRICS_V1, calibrated=True, cal_index=cal_idx,
    )
...
def _benchmark_cohort_data() -> tuple[dict[str, dict[str, float | None]], dict[str, dict[str, float]], int, int]:
    """Layer-1 + Layer-2 values for BKH, VA v1, VA v2."""
    bkh_stats = _load_bkh_stats_dict()
    va_stats = _load_golden_stats()
    bkh_h = _golden_heuristic_retrieval_from_stats(bkh_stats) if bkh_stats else _bkh_heuristic_live()
    ...
    layer2_keys = {k for k, _, _ in GOLDEN_LLM_METRICS_V1} | {k for k, _, _ in COMPARISON_LAYER2_METRICS}
...
def fig_ab_side_by_side() -> Path:
    ...
    ax.plot(
        x, thresholds, "o--", color=AMBER, linewidth=1.8, markersize=5, zorder=4, label="Threshold",
    )
    ax.set_xticks(x)
    ...
    *,
    ...
    for si, cohort in enumerate(_COHORT_ORDER):
        for mi, (key, _, _) in enumerate(metrics):
            ...
            bar = ax.bar(
                bx, v, w, color=_COHORT_COLORS[cohort], alpha=0.88, zorder=3,
                label=cohort if show_label else None,
            )
            if show_label:
                legend_seen.add(cohort)
            ax.text(
                bar[0].get_x() + bar[0].get_width() / 2, max(v, 0) + 1.5, f"{v:.0f}",
                ha="center", va="bottom", fontsize=7, color=NAVY, fontweight="bold",
            )

    ax.plot(
        x, thresholds, "o--", color=AMBER, linewidth=1.8, markersize=4, zorder=4,
        label="Threshold",
    )
...
def _horizontal_cohort_panel(
    ...
    *,
    ...
    for mi, (key, label, thr) in enumerate(metrics):
        y0 = y_centers[mi]
        for ci, cohort in enumerate(_COHORT_ORDER):
            ...
            ax.text(
                val + 1.2, y, f"{val:.0f}%",
                va="center", ha="left", fontsize=7.5, color=NAVY, fontweight="bold",
            )
            if show_thresholds and thr is not None:
                ax.plot([thr, thr], [y - bar_h * 0.44, y + bar_h * 0.44],
                        color=AMBER, linewidth=1.2, linestyle="--", zorder=2, alpha=0.85)
    ...
    *,
    ...
    if include_layer1:
        ...
        axes[ai].set_title(
            ...
            fontweight="bold", color=NAVY, fontsize=10, loc="left", pad=8,
        )
        ai += 1
    ...
    if include_layer2:
        ...
        axes[ai].set_title(
            f"Layer 2 — primary LLM gates "
            f"(BKH cal n={bkh_n} · VA n={n_va} · grounding N/A on BKH)",
            fontweight="bold", color=NAVY, fontsize=10, loc="left", pad=8,
        )
    ...
    fig.legend(handles=handles, loc="lower center", ncol=3, fontsize=9, framealpha=0.95, bbox_to_anchor=(0.5, 0.01))
...
def fig_heuristic_llm_compare() -> Path:
    ...
    ax.bar(x - w / 2, bkh_vals, w, label=f"BKH (n={bh['n_rated']:,} rated)", color=NAVY, alpha=0.85, zorder=3)
    ax.bar(x + w / 2, g_vals, w, label=f"VA Golden (n={gh['n_rated']:,} rated)", color=TEAL, alpha=0.85, zorder=3)
    ...
    gs_cal = compute_llm_pass_summary(q_v1, GOLDEN_LLM_METRICS_V1, calibrated=True, cal_index=cal_idx)
    ...
    colors = [GREEN if r >= t else RED for r, t in zip(g_rates, tholds)]
...
def fig_power_curve() -> Path:
    ...
    ax.axhline(0.02, color=GREEN, linewidth=1.8, linestyle="--",
               label="Merge gate: ΔMRR ≥ 0.02")
...
def fig_bkh_overview() -> Path:
    ...
    ax2 = axes[1]
    ft_labels = ["Unrated", "A_retrieval\n(no KB ans.)", "B_language\n(lang mismatch)",
                 "E_grounding\n(cited+disliked)", "C_friction", "no_failure\n(liked)"]
    ft_sizes  = [79.6, 15.1, 4.2, 0.6, 0.2, 0.3]
    ft_colors = [SLATE, AMBER, PURPLE, RED, "#f97316", GREEN]
    wedges2, texts2, autotexts2 = ax2.pie(
        ft_sizes, labels=ft_labels, colors=ft_colors, autopct="%1.1f%%",
    ...
    fig.suptitle("BKH Dataset — 69,198 turns · 30,557 conversations · 1.7% rated",
                 fontsize=12, color=NAVY, fontweight="bold", y=1.02)
    fig.tight_layout()
...
def fig_topic_sentiment(
    ...
    *,
    ...
    like_items = sorted(
        [(t, v) for t, v in cats.items() if v.get("count", 0) >= 5 and "like_rate" in v and t not in _excl],
    ...
    if not dislike_items:
        dislike_items = [
            ("Managing Accounting Entries",              {"dislike_rate": 0.035, "sources_rate": 0.64, "count": 200}),
            ("Human Support Chat Assistance",            {"dislike_rate": 0.034, "sources_rate": 0.54, "count": 932}),
            ("Account Usage and Differences",            {"dislike_rate": 0.033, "sources_rate": 0.34, "count":  61}),
            ("Invoice Sending and Management Issues",    {"dislike_rate": 0.032, "sources_rate": 0.63, "count": 281}),
            ("Bank Connection Issues",                   {"dislike_rate": 0.030, "sources_rate": 0.62, "count": 168}),
            ("Billy Accounting and Payment Integration", {"dislike_rate": 0.029, "sources_rate": 0.70, "count": 244}),
            ("Creating and Managing Account Plans",      {"dislike_rate": 0.028, "sources_rate": 0.73, "count": 143}),
            ("Bank Reconciliation Issues",               {"dislike_rate": 0.023, "sources_rate": 0.76, "count":  87}),
            ("Tax Reporting of Financial Results",       {"dislike_rate": 0.019, "sources_rate": 0.76, "count":  53}),
            ("Invoice Payment and Reconciliation",       {"dislike_rate": 0.018, "sources_rate": 0.75, "count":  57}),
        ]
    ...
    for i, (rate, src) in enumerate(zip(d_rates, d_src)):
        ax.text(rate + 0.05, i, f"{rate:.1f}%", va="center", fontsize=8, color=RED, fontweight="bold")
    ...
    if like_items:
        ...
        for i, (rate, src) in enumerate(zip(l_rates, l_src)):
            ax2.text(rate + 0.05, i, f"{rate:.1f}%", va="center", fontsize=8, color=GREEN, fontweight="bold")
        ax2.legend(fontsize=8, loc="lower right", framealpha=0.9)
    else:
        ax2.text(0.5, 0.5, "Run eval_stats to\ngenerate liked-topic data",
                 ha="center", va="center", transform=ax2.transAxes, color=MID, fontsize=11)
        ax2.set_title("Top 10 Liked Topics", color=NAVY, fontweight="bold")
...
def _draw_retrieval_metrics_panel(
    ...
    *,
    ...
    if alt is not None:
        ...
        for bar, v in zip(bars, ax2_vals):
            if v > 0:
                ax.text(bar.get_x() + bar.get_width() / 2, v + 1.5, f"{v:.0f}",
                        ha="center", va="bottom", fontsize=7, color=NAVY, fontweight="bold")
    ...
    if alt_footnote:
        ax.text(0.5, -0.20, alt_footnote, ha="center", va="top", transform=ax.transAxes,
    ...
    *,
    ...
    for si, key in enumerate(order):
        ...
        for bar, v in zip(bars, vals):
            if v > 0:
                ax.text(
                    bar.get_x() + bar.get_width() / 2, v + 1.5, f"{v:.0f}",
                    ha="center", va="bottom", fontsize=7, color=NAVY, fontweight="bold",
                )
    ...
    if footnote:
        ax.text(
            0.5, -0.22, footnote, ha="center", va="top", transform=ax.transAxes,
            fontsize=7.5, color=MID, style="italic",
        )
    ...
    *,
    ...
    if cohort_scores is None and file_stats:
        cohort_scores = _build_cohort_scores(
            file_stats, rp_data, use_full_recall=False,
        )
    ...
    subs = cell_subs or (
        "liked + sourced", "liked + no source",
        "disliked + sourced", "disliked + no source",
    )
    ...
    for x, y, label, sub, bg, fg in cell_data:
        ...
        ax.text(x + 0.5, y + 0.64, label, ha="center", va="center",
                fontsize=12, color=fg, fontweight="bold", zorder=3)
        ax.text(x + 0.5, y + 0.28, sub, ha="center", va="center",
    ...
    ax.set_title(
        ...
        color=NAVY, fontweight="bold", pad=28, fontsize=11,
    )
    ax.text(0.5, 2.05, "(rated turns only, unrated excluded)",
            ha="center", va="bottom", fontsize=8, color=MID, transform=ax.transData)
...
_LANG_DISPLAY = {"da": "Danish", "en": "English", "de": "German", "unknown": "Unknown", "nl": "Dutch"}
...
def fig_golden_response_type(
    ...
    *,
    ...
    for i, key in enumerate(("has_sources", "unknown", "escalation", "clarification", "interrupted")):
        if key not in rt:
            continue
    ...
    *,
    ...
    if ft_order is None:
        ...
        if n_rated_pct and n_rated_pct < 0.1:
            ft_order = [
                "n/a", "unknown", "A_retrieval", "B_language",
                "E_grounding", "C_friction", "no_failure", "unclassified", "D_scope",
            ]
        else:
            ft_order = [
                "unclassified", "no_failure", "E_grounding", "n/a",
                "A_retrieval", "B_language", "C_friction", "D_scope", "unknown",
            ]
    labels, sizes, colors = [], [], []
    for key in ft_order:
        if key not in ft:
...
def fig_golden_source_count(
    ...
    *,
    ...
    bar_colors = [AMBER if k == "0" else TEAL if k == "1" else NAVY if k == "2" else PURPLE for k in keys]
...
def fig_va_retrieval_proxy_v3() -> Path:
    ...
    bkh_scores = _build_cohort_scores(
        bkh_stats, bkh_stats.get("retrieval_proxy"), use_full_recall=True,
    )
    scenarios = {
    ...
    ax.text(
        ...
        ha="center", va="top", fontsize=8, color=NAVY, transform=ax.transAxes, fontweight="bold",
    )
    ...
    fig.suptitle(
        ...
        fontsize=11, color=NAVY, fontweight="bold", y=0.98,
    )
...
def export_all(fig_names: list[str] | None = None) -> None:
    ...
    _GOLDEN_FIGS = {
        "golden_response_type", "golden_failure_mode",
        "golden_source_count", "golden_language", "golden_retrieval_proxy",
    }
    golden_stats = None
    ...
    for name in figures_to_run:
        ...
        try:
            if name in _GOLDEN_FIGS:
                ...
                fn(bkh_stats)
            elif name == "golden_pass_rates_v1":
                fn()
            elif name == "golden_pass_rates_v2":
                fn()
            elif name in ("golden_pass_rates", "va_pass_rates"):
                fn()
            else:
                fn()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fig", nargs="+", choices=list(ALL_FIGURES.keys()),
