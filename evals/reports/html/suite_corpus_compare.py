...
def _load_stats_blob(corpus: str) -> dict:
    p = va_staging_all_responses_stats_path() if corpus == "va" else bkh_stats_path()
    if not p.exists():
        return {}
...
def suite_corpus_compare_html(report: SuiteReport) -> str:
    ...
    for label, bkh_v, va_v, bn, vn in h_rows:
        ...
        delta = f"{(va_v - bkh_v):+.1%}" if bkh_v is not None and va_v is not None else "—"
        body += (
    ...
    for _key, label, thr_pct in COMPARISON_LAYER2_METRICS:
        ...
        delta = f"{va_pct - bkh_pct:+.0f}" if bkh_pct is not None and va_pct is not None else "—"
        d_cls = ""
