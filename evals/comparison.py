...
from evals.arima import ARIMAForecaster
...
class ModelComparisonResult:
    ...
    def print_table(self) -> None:
        try:
            ...
            for name, s in self.summary_by_model().items():
                print(
                    f"{name}: MASE={s['mean_mase']:.3f} SMAPE={s['mean_smape']:.1f}% "
...
def _score_fold_arima(
    ...
    val_vals = (
        ...
        .sort("date")
        .head(horizon_days)["amount" if "amount" in fold.val.columns else "value"]
        .to_numpy()
        .astype(float)
    )
    ...
    try:
        ...
        # Directional
        dir_acc = (
            float(100.0 * np.mean(np.sign(np.diff(actuals)) == np.sign(np.diff(preds))))
            if len(actuals) > 1
            else 50.0
        )
...
def _run_chronos_or_fallback(
    ...
    try:
        ...
        quantiles, mean = pipeline.predict_quantiles(
            ctx, prediction_length=horizon, quantile_levels=[0.1, 0.5, 0.9], num_samples=50
        )
        return (
            mean[0].numpy().tolist(),
    ...
    try:
        ...
        return (
            pred["AutoETS"].tolist(),
            pred["AutoETS-lo-80"].tolist(),
            pred["AutoETS-hi-80"].tolist(),
        )
    except Exception:
        pass
